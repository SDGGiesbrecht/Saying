...
struct Module {
  ...
  func build() throws {
#warning("Debugging...")
let source: StrictString = "(\u{2028}abc: def\u{2028})"
assert((try? ParsedThingName.diagnosticParse(source: source).get())?.source() == source)
assert(ParsedThingName.fastParse(source: source)?.source() == source)
